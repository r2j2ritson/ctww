    ...
    if isinstance(url, str):
        with detection_graph.as_default():
            with tf.Session(graph=detection_graph) as sess:
                #img_file = BytesIO(urlopen.urlopen(url).read()) #only required if downloading from an HTTPS
                img_file = url
...
def run_megadetector(MODEL_FILE, IMAGE_PATH, OUT_PATH):
  IMAGES = os.listdir(IMAGE_PATH)
  TARGET_IMAGES = [IMAGE_PATH + f for f in IMAGES]
  detection_graph = load_model(MODEL_FILE) #First Core Function
  
  #startTime = time.time()
  #boxes,scores,classes,images = generate_image_detections(detection_graph,TARGET_IMAGES)
  ...
  for img_file in TARGET_IMAGES:
     box, score, clss = generate_image_detections(detection_graph, img_file) #Second Core Function
     name, ext = os.path.splitext(img_file.split('/')[-1])
     num_objects, bboxes = draw_image_detections(box, score, clss, img_file, OUT_PATH + name ) #Third Core Function
  ...
  import json
  with open(OUT_PATH + 'results.json', 'w') as res:
